...
% 	\endgroup
	...
	\noindent \textsc{Apuntes de la asignatura \emph{Teoría de circuitos} realizados por Ana Fernández Guillamón. Se trata de una asignatura común a todos los Grados impartidos en la Escuela Técnica Superior de Ingeniería y Diseño Industrial}\\ % License information
	...
	\chapterimage{tema1.jpg} % Chapter heading image
	...
	\begin{example}\label{ex.1-2}
	    ...
	    \begin{figure}[H]
	        \centering
	        \subfigure[]{\includegraphics[height=3.7cm]{../figs/Conversion_Fuentes.pdf}}\hfil
	        \subfigure[]{\includegraphics[height=3.7cm]{../figs/Conversion_Fuentes_2.pdf}}\hfil
	        \subfigure[]{\includegraphics[height=3.7cm]{../figs/Conversion_Fuentes_3.pdf}}\hfil
	        \subfigure[]{\includegraphics[height=3.7cm]{../figs/Conversion_Fuentes_4.pdf}}
	...
	\begin{example}\label{ex.motor-bt1}
	    ...
	    El circuito eléctrico equivalente se muestra en la Figura~\ref{fig.ejemplo_BT1_motor}.
	    \begin{figure}[H]
	        \centering
	        \includegraphics[width=0.4\linewidth]{../figs/ejemplo_BT1_motor.pdf}
...
% 		\subfigure[Conexión estrella]{\includegraphics[width=0.3\linewidth]{../figs/ej7b_BT1.pdf}}
...
donde se cumple que la suma es 0.
	...
	\subsubsection{Método de los nudos modificados}
	...
	\begin{example}
		...
		\begin{itemize}
			\item $\overline{U}=150\phase{45^\circ}$ V
			\item $\overline{I}=3\phase{30^\circ}$ A
		\end{itemize}
	\end{example}
...
\end{figure}
\item En el circuito de la Figura~\ref{fig.ej10_BT1}, donde se sabe que la carga inicial de los condensadores era 10 $\mu$C (para el de 10 $\mu$F) y 20 $\mu$C (para el de 20 $\mu$F) con las polaridades indicadas, se pide determinar:
\begin{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{../figs/ej10_BT1.pdf}
...
\item El circuito de la Figura~\ref{fig.mallas_condensadores_bobinas} está funcionando en régimen estacionario. Los condensadores estaban inicialmente descargados. Resuelve el circuito mediante el método que consideres conveniente para obtener los siguientes resultados:
\begin{itemize}
\item Las intensidades señaladas.
\item Polaridad y energía almacenada en los condensadores.
\item Balance de potencias.
\end{itemize}
Datos: $\epsilon_{1}={40}V; \epsilon_{2}={22}V; \epsilon_{3}={20}V; C_{1}=C_{2}=C_{3}={2}{\mu F}; R_{g1}=R_{g2}=R_{g3}={4}{\Omega}; R_{1}=R_{2}=R_{3}=R_{4}={2}{\Omega}; R_{5}=R_{6}=R_{7}={1}{\Omega}$
    
    \emph{Sol.: $I_1=I_5=2A; I_2=I_3=I_8=I_{10}=-1A; I_4=I_7=I_{11}=I_{12}=I_{13}=0A; I_6=I_{14}=1A; E_{C1}=0.676 mJ; E_{C2}=0.576 mJ; E_{C3}=1\mu J; \sum P = 0$}
  \begin{figure}[htbp]
      \centering
      \includegraphics{../figs/mallas_condensadores_bobinas.pdf}
...
\item En el circuito de la Figura~\ref{fig:formas_onda1} hay que determinar:
...
\end{itemize}
Datos: $R = {5}{\Omega}$, $C = {0.2}{F}$

\emph{Sol.: $i_1(t) =   \begin{cases}
    5 & 0 \leq t < 1\\
    -5 & 1 \leq t < 2 
  \end{cases};\,p_E(t) =                \begin{cases}
                 50\cdot t & 0 \leq t < 1\\
                 50 \cdot t - 100 & 1 \leq t < 2 
               \end{cases};\,P_R = {45} W$}

\begin{figure}[H]
    \includegraphics[width=0.23\linewidth]{../figs/tren_pulsos.pdf}
    \includegraphics[width=0.45\linewidth]{../figs/formas_onda1.pdf}
    \includegraphics[width=0.25\linewidth]{../figs/onda_triangular.pdf}
    \caption{Ejercicio 16}
    \label{fig:formas_onda1}
\end{figure}

\item En el circuito de la Figura~\ref{fig:formas_onda2} se pide:
...
\item Obtener la expresión analítica de la corriente $i(t)$ y de la potencia del generador $e(t)$.
\end{itemize}
Datos: $R = {6}{\Omega}$, $L = {4}{mH}$, $E = {12}{V}$
...
\emph{Sol.: $i_R(t) = 
      ...
      \end{cases};\;I_{Rm}=2A;\;I_R=2A;\;I_{Lm}=1.5\,A;\;I_L=\sqrt{3} A$}
\begin{figure}[H]
 \centering
    \includegraphics[width=0.35\linewidth]{../figs/formas_onda2.pdf}\hfil
    \includegraphics[width=0.25\linewidth]{../figs/tren_pulsos2.pdf}
    \caption{Ejercicio 17}
    \label{fig:formas_onda2}
\end{figure}
...
\end{enumerate}
	...
	\vspace{4mm}
	\begin{example}
		\textbf{Sabiendo que $i_1(t)=2\cdot\sqrt{2}\cdot \sin(600\cdot t)$ A; $i_2(t)=4\cdot\sqrt{2}\cdot \sin(600\cdot t+\frac{\pi}{2})$ A; e $i_3(t)=10\cdot\sqrt{2}\cdot \cos(600\cdot t-\pi)$ A, determinar la suma de $i_1(t)+i_2(t)+i_3(t)$.}
		...
		\end{equation*}
		\begin{equation*}
			i_2(t)=4\cdot\sqrt{2}\cdot \sin(600\cdot t+\frac{\pi}{2})\rightarrow \overline{I_2}=4\phase{90^\circ}\;\text{A}\
		\end{equation*}
		\begin{equation*}
			i_3(t)=10\cdot\sqrt{2}\cdot \cos(600\cdot t-\pi)=10\cdot\sqrt{2}\cdot \cos(600\cdot t-\frac{\pi}{2})\rightarrow \overline{I_3}=10\phase{-90^\circ}\;\text{A}\
		\end{equation*}
	...
	\begin{example}\label{ex.impedancia_admitancia_eq}
	    ...
	    \begin{figure}[H]
	        \centering
	        \includegraphics[width=0.4\linewidth]{../figs/impedancia_admitancia_eq.pdf}
	    ...
	    \begin{equation*}
	        \overline{Y_{eq}}=\dfrac{1}{\ovelrine{Z_{eq}}}=\dfrac{1}{18.61\phase{7.1250^\circ}}=0.05\phase{-7.1250^\circ} S
	    \end{equation*}
	\end{example}
	...
	\begin{example}\label{ej.2-6}
		\textbf{En el circuito de la Figura \ref{fig.ejercicio6_tema01}, determinar la tensión $U$ que hace que la corriente que circula por la impedancia $2+\mathrm{j}3\,\Omega$ sea nula.}
		\begin{figure}[H]
			\centering
	...
	\begin{example}\label{ej.2-7}
		...
		Se pasa $u_1(t)$ a función senoidal, obteniéndose $u_1(t)=10\sqrt{2}\cdot \sin(1000\cdot t+\frac{\pi}{2})$. Así, los fasores de $\overline{U_1}$ y $\overline{U_2}$ son:
		\begin{align*}
...
\end{align*}
\begin{figure}[htbp]
    \centering\subfigure[Líneas $A$ y $B$]{\includegraphics[width=0.3\linewidth]{../figs/Potencia_3H_equilibrado_AB.pdf}\label{fig.Potencia3H_Equilibrado_AB}}\hfill
    \subfigure[Líneas $B$ y $C$]{\includegraphics[width=0.3\linewidth]{../figs/Potencia_3H_Equilibrado_BC.pdf}\label{fig.Potencia3H_Equilibrado_BC}}\hfill
    \subfigure[Líneas $C$ y $A$]{\includegraphics[width=0.3\linewidth]{../figs/Potencia_3H_Equilibrado_CA.pdf}\label{fig.Potencia3H_Equilibrado_CA}}
...
\begin{figure}[htbp]
    \centering
    \centering\subfigure[Líneas $A$ y $B$]{\includegraphics[width=0.3\linewidth]{../figs/Potencia3H_Equilibrado_AB_SFI.pdf}\label{fig.Potencia3H_Equilibrado_AB_SFI}}\hfill
    \subfigure[Líneas $B$ y $C$]{\includegraphics[width=0.3\linewidth]{../figs/Potencia3H_Equilibrado_BC_SFI.pdf}\label{fig.Potencia3H_Equilibrado_BC_SFI}}\hfill
    \subfigure[Líneas $C$ y $A$]{\includegraphics[width=0.3\linewidth]{../figs/Potencia3H_Equilibrado_CA_SFI.pdf}\label{fig.Potencia3H_Equilibrado_CA_SFI}}
...
\begin{itemize}
    ...
    \item Si la línea de alimentación a la carga dispone de una impedancia $\overline{Z_L}=1+\mathrm{j}$ por conductor, determinar la tensión en forma fasorial que debe disponer el generador en sus bornas para que
la tensión en la carga sea 380V
\end{itemize}}
\begin{figure}[H]
    \centering
...
\end{equation*}
que, en forma fasorial, sabiendo que $\theta_U=30+\phi_T+\theta_I$ (puesto que los ángulos son de las fases):
\begin{align*}
    \overline{U_A}&=490.72\phase{121.8436^\circ} V\\
    \overline{U_B}&=490.72\phase{1.8436^\circ} V\\
    \overline{U_C}&=490.72\phase{-118.1564^\circ} V
\end{align*}
\end{example}
		...
		\begin{figure}
		    \centering
		    \subfigure[Triángulo]{\includegraphics[width=0.35\linewidth]{../figs/CircuitoTrifasica_CompensacionReactiva.pdf}\label{fig.compensacion_reactiva_triangulo_trif}}\hfill
		    \subfigure[Estrella]{\includegraphics[width=0.4\linewidth]{../figs/CircuitoTrifasicaY_CompensacionReactiva.pdf}\label{fig.compensacionY_reactiva_triangulo_trif}}
	...
	% \begin{example}
		...
		% \begin{equation*}
			%     E_{a,F}=\dfrac{E_{ab}-E_{bc}}{3}=\dfrac{240\phase 0-240\phase -120}{3}=138.56\phase 30^\circ\;V
			% \end{equation*}
		...
		% \begin{equation*}
			%     I_{a,F}=\dfrac{138.56\phase 30^\circ}{(1.8\phase 68.2)+(1+j\sqrt{3})+(2.13\phase -51.34)}=40\phase 0 \;\text{A}
			% \end{equation*}
...
% \end{equation*}
	...
	\begin{example}
	    ...
	    \begin{figure}[H]
	        \centering
	        \includegraphics[width=0.45\linewidth]{../figs/ejemplo_2orden.pdf}
	        \captionof{figure}{Ejemplo ZXX}
	    ...
	    \begin{figure}[H]
	        \centering
	        \includegraphics[width=0.45\linewidth]{../figs/ejemplo_2orden1.pdf}
	        \captionof{figure}{Ejemplo ZXX}
	...
	\end{equation*}
	Por tanto, dado que $i(t)=\frac{U_A}{1}$, se obtiene que:
	\begin{equation*}
