...
\item En el circuito de la figura, los condensadores se conectaron sin
  ...
  \emph{Sol.:
    ...
    U_C=42V; U_D=150V; U_{C1}=0V; q_1=0\mu C; E_{C1}=0J; U_{C2}=-42V;
    q_2=84\mu C; E_{C2}=1.76 mJ; U_{C3}=-42V; q_3=84\mu C; E_{C3}=1.76
    mJ; P_g = P_R$}
...
\item En el circuito de la figura se debe determinar:
  ...
  \emph{Sol.:
    $I_1=I_2=1 A; I_3=I_4=0 A; I_5=-2 A; \sum_\epsilon P_\epsilon = \sum_R P_R; U_A=-1 V; U_B=-5
...
\item Obtener el generador equivalente de Thévenin del circuito de la
  ...
  \item Obtener el generador equivalente de Thévenin del circuito de la figura respecto de A y B.\\
    Datos: $I_g=10\,\text{A}; R_1=1\,\Omega;\; \alpha=5$
    \begin{center}
      \includegraphics[height=4.5cm]{../figs/Thevenin1.pdf}
    \end{center}

    \emph{Sol.: $\epsilon_{th}=\qty{60}{\volt};\;R_{th}=6\Omega$}
  ...
  \item En el circuito de la figura, calcular:
    ...
    Datos:
    $R = {1}{\Omega};\; \epsilon_g = {10}{V};\; \alpha = \qty{2}{\ohm};\; \beta = 1$
...
\emph{Sol.:
  $I_N=\frac{10}{3}\,\text{A};\; R_N=2\,\Omega;\; R_L=2\,\Omega;\;
  P_L=5.56\,\text{W}$}
