...
- El instante del cambio se representa habitualmente con $t = 0$:
  - $t = 0^-$: la topología del circuito es la anterior al cambio.
  - $t = 0^+$: la topología del circuito es la posterior al cambio.
*** Resistencia
#+BEGIN_EXPORT latex
\[
u(t) = R i(t)
