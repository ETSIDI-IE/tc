...
#+BEGIN_EXPORT latex
\[
  \laplace{F} = \frac{K_1}{\slp-p_1} + \frac{K_2}{\slp-p_2} + \ldots + \frac{K_n}{\slp-p_n} 
\]
#+END_EXPORT

#+BEGIN_EXPORT latex
\[
  K_i = \left[(\slp - p_1) \laplace{F}\right]_{s = p_i}
\]
#+END_EXPORT
...
- Método algebraico (véase ejemplo 15.9 de AS)
...
- Completar cuadrados + Método algebraico 
*** Ejemplo 15.11 AS
#+BEGIN_EXPORT latex
\[
...
** Ejemplo 8.7 AS (resuelto con análisis clásico)

[[file:figs/Ejemplo_8-7_AS.pdf]]

#+BEGIN_EXPORT latex
\begin{align*}
  i_L(t) &= \frac{4}{3} \left(4 e^{-t} - e^{-4t} \right)\\
  u_C(t) &= 24 + \frac{4}{3} \left(-16 e^{-t} + e^{-4t} \right)\\
\end{align*}
#+END_EXPORT
...
[[file:figs/Ejemplo_PoleZero.pdf]]


#+BEGIN_EXPORT latex
\[
  \laplace{H} = \frac{3\slp + 6}{\slp^3 + 3\slp^2 + 7\slp + 5}
\]
#+END_EXPORT
