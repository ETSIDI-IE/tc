...
\label{sec:org6e0eead}

\subsection{¿Qué es el régimen transitorio?}
\label{sec:orgca0574d}
\begin{frame}[label={sec:orgfbc6e78}]{Permanente y Transitorio}
...
\begin{frame}[label={sec:org78f7cd1}]{Acumulación de Energía}
...
\label{sec:org4d34c1a}
\begin{frame}[label={sec:org55e7500}]{Respuesta completa de una red lineal}
...
\begin{frame}[label={sec:org535ebe2}]{Condiciones iniciales}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org6f8748a}]{Resistencia}
\[
u(t) = R i(t)
\]
...
\begin{frame}[label={sec:org120c980}]{Inductancia}
\[
u(t) = L \diff{i_L(t)}{t}
\leftrightarrow
...
\begin{frame}[label={sec:org71415c8}]{Capacidad}
\[
i(t) = C \diff{u_C(t)}{t}
\leftrightarrow
...
\]
\end{frame}
\begin{frame}[label={sec:orgdaa4ef7}]{Ejemplo}
...
\label{sec:orgde14f10}
\begin{frame}[label={sec:orga1ae987}]{Definición}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org3f7dc73}]{Respuesta natural y forzada}
\begin{itemize}
...
\label{sec:orgdf88e3d}

\begin{frame}[label={sec:org379ee8d}]{Circuito básico}
\begin{itemize}
...
\begin{frame}[label={sec:orgf6c5920}]{Respuesta natural}
\begin{center}
...
\begin{frame}[label={sec:orgfdfb459}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:org862d959}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:org43a3f5a}]{Constante de tiempo}
\[
...
\begin{frame}[label={sec:org57baf60}]{Balance Energético}
...
\begin{frame}[label={sec:orge67a2e9}]{Respuesta forzada}
...
\begin{frame}[label={sec:orge0e0cd1}]{Respuesta forzada}
\begin{center}
...
\begin{frame}[label={sec:orgf1f7bba}]{Condiciones iniciales}
...
\begin{frame}[label={sec:orgee42a17}]{Respuesta completa (ejemplo)}
\begin{align*}
  i(t) &= i_n(t) + i_\infty(t)\\
  i_n(t) &= A e^{st}\\
  i_\infty(t) &= U_0/R\\
...
\begin{frame}[label={sec:org503a131}]{Respuesta completa}
\[
  \boxed{i(t) = \frac{U_0}{R}(1 - e^{-\frac{t}{\tau}})  }
\]
...
\begin{frame}[label={sec:org809d60e}]{Expresión general de la respuesta completa}
\[
...
\begin{frame}[label={sec:orgefe7140}]{Equivalente de Thévenin}
\begin{center}
\includegraphics[height=0.85\textheight]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}
\subsection{Circuito RC paralelo}
\label{sec:orgc0dde64}

\begin{frame}[label={sec:orgfbebc07}]{Circuito básico}
\begin{itemize}
...
\begin{frame}[label={sec:org73e5eb9}]{Respuesta natural}
\begin{center}
...
\begin{frame}[label={sec:org5843079}]{Constante de tiempo}
\begin{itemize}
...
\begin{frame}[label={sec:orgf053825}]{Balance Energético}
...
\begin{frame}[label={sec:org95beab0}]{Expresión general de la respuesta completa}
\[
...
\begin{frame}[label={sec:org1b4247d}]{Ejemplo con respuesta forzada}
\begin{center}
...
\begin{frame}[label={sec:orgf63b102}]{Equivalente de Norton}
\begin{center}
\includegraphics[height=0.85\textheight]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}
...
\label{sec:org349ee45}

\begin{frame}[label={sec:org80b0c1c}]{Equivalente de Thévenin/Norton}
\begin{center}
\includegraphics[height=0.85\textheight]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org25238f5}]{Procedimiento General}
\begin{itemize}
...
\label{sec:org6ba0dcd}
\begin{frame}[label={sec:org8b05a2f}]{Introducción}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb88acd0}]{Respuesta natural y forzada}
\begin{itemize}
\item El método de resolución analiza el circuito en dos etapas:
\begin{itemize}
...
\label{sec:org31c8e28}
\begin{frame}[label={sec:org057e3a3}]{Circuito básico}
\begin{center}
...
\begin{frame}[label={sec:orgee1a52d}]{Respuesta natural (t > 0)}
\begin{center}
...
\begin{frame}[label={sec:orgba225d8}]{Solución}
...
\begin{frame}[label={sec:org44c6729}]{Parámetros}
\begin{columns}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgff1e1c4}]{Posibles soluciones}
\[
...
\begin{frame}[label={sec:orge7adaa7}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:orga29b4eb}]{Circuito Sobreamortiguado (\(\alpha > \omega_0\))}
\[
...
\begin{frame}[label={sec:orgc10c899}]{Amortiguamiento Crítico (\(\alpha = \omega_0\))}
\[
...
\begin{frame}[label={sec:org66b813a}]{Circuito Subamortiguado (\(\alpha < \omega\))}
\[
...
\begin{frame}[label={sec:org2017156}]{Valores Importantes}
\begin{center}
...
\begin{frame}[label={sec:orge069021}]{Valores Importantes}
\begin{center}
\includegraphics[height=0.6\textheight]{figs/RespuestaEscalon_SegundoOrden.png}
\end{center}
...
\begin{frame}[label={sec:org58997c5}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:org4412b9e}]{Derivadas en \(t = 0^+\)}
\begin{center}
...
\begin{frame}[label={sec:orgc9134ba}]{Respuesta Completa}
...
\begin{frame}[label={sec:orga42de5c}]{Ejemplo de Respuesta Completa}
...
\label{sec:orgbc1ceaa}
\begin{frame}[label={sec:org6784332}]{Circuito básico}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/transitorio_circuitoRLC_paralelo.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org73175a9}]{Respuesta natural (t > 0)}
\begin{center}
...
\begin{frame}[label={sec:orge451649}]{Solución}
\begin{block}{Ecuación diferencial}
\[
  \diff[2]{u}{t} + \frac{G}{C} \diff{u}{t} + \frac{1}{LC} u = 0
\]
...
\begin{frame}[label={sec:orgb019a79}]{Parámetros}
\begin{block}{Ecuación característica}
\begin{columns}
\begin{column}{.5\columnwidth}
\begin{align*}
...
\begin{frame}[label={sec:org5ab17d4}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:org49ab671}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:orgc810b74}]{Condiciones Iniciales}
\begin{block}{Dos constantes a determinar}
Son necesarias dos tipos de condiciones iniciales:
...
\begin{frame}[label={sec:orgc72db03}]{Derivadas en \(t = 0^+\): ejemplo RLC paralelo}
\begin{center}
...
\begin{frame}[label={sec:org09e9544}]{Respuesta Completa}
Las condiciones iniciales deben evaluarse teniendo en cuenta la respuesta forzada (si existe).
\begin{align*}
...
\begin{frame}[label={sec:org85b3f1c}]{Ejemplo de Respuesta Completa}
