...
\label{sec:org3d62cea}

\subsection{¿Qué es el régimen transitorio?}
\label{sec:orgca6801f}
\begin{frame}[label={sec:org5945755}]{Permanente y Transitorio}
...
\begin{frame}[label={sec:orgc79e287}]{Acumulación de Energía}
...
\label{sec:org51eb78c}
\begin{frame}[label={sec:orgd011b17}]{Respuesta completa de una red lineal}
...
\begin{frame}[label={sec:org11c4039}]{Condiciones iniciales}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd47fcfb}]{Resistencia}
\[
u(t) = R i(t)
\]
...
\begin{frame}[label={sec:org2f838d1}]{Inductancia}
\[
u(t) = L \diff{i_L(t)}{t}
\leftrightarrow
...
\begin{frame}[label={sec:org94dd6a2}]{Capacidad}
\[
i(t) = C \diff{u_C(t)}{t}
\leftrightarrow
...
\]
\end{frame}
\begin{frame}[label={sec:orgd7d5561}]{Ejemplo}
...
\label{sec:org42e6822}
\begin{frame}[label={sec:orgf75e970}]{Definición}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc3d9794}]{Respuesta natural y forzada}
\begin{itemize}
...
\label{sec:orgb2c3d45}

\begin{frame}[label={sec:orgfaedbea}]{Circuito básico}
\begin{itemize}
...
\begin{frame}[label={sec:org6958b39}]{Respuesta natural}
\begin{center}
...
\begin{frame}[label={sec:orgbc38256}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:orgdd25832}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:org06abc1c}]{Constante de tiempo}
\[
...
\begin{frame}[label={sec:orgdcc8bff}]{Balance Energético}
...
\begin{frame}[label={sec:orgeceb3fc}]{Respuesta forzada}
...
\begin{frame}[label={sec:org03af115}]{Respuesta forzada}
\begin{center}
...
\begin{frame}[label={sec:orga934690}]{Condiciones iniciales}
...
\begin{frame}[label={sec:org5d29351}]{Respuesta completa (ejemplo)}
\begin{align*}
  i(t) &= i_n(t) + i_\infty(t)\\
  i_n(t) &= A e^{st}\\
  i_\infty(t) &= U_0/R\\
...
\begin{frame}[label={sec:orgcee398f}]{Respuesta completa}
\[
  \boxed{i(t) = \frac{U_0}{R}(1 - e^{-\frac{t}{\tau}})  }
\]
...
\begin{frame}[label={sec:orgc32feee}]{Expresión general de la respuesta completa}
\[
...
\begin{frame}[label={sec:org522c7b4}]{Equivalente de Thévenin}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}
\subsection{Circuito RC paralelo}
\label{sec:org9e59079}

\begin{frame}[label={sec:org8af52a4}]{Circuito básico}
\begin{itemize}
...
\begin{frame}[label={sec:orgd4161e7}]{Respuesta natural}
\begin{center}
...
\begin{frame}[label={sec:orgb09fac2}]{Constante de tiempo}
\begin{itemize}
...
\begin{frame}[label={sec:orgc3209a9}]{Balance Energético}
...
\begin{frame}[label={sec:orged1e523}]{Expresión general de la respuesta completa}
\[
...
\begin{frame}[label={sec:orgaa2e46e}]{Ejemplo con respuesta forzada}
\begin{center}
...
\begin{frame}[label={sec:orge4c48c1}]{Equivalente de Norton}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}
...
\label{sec:orge625e82}

\begin{frame}[label={sec:org205eba7}]{Equivalente de Thévenin/Norton}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgf774bc6}]{Procedimiento General}
\begin{itemize}
...
\label{sec:org234d2a6}
\begin{frame}[label={sec:org95f6841}]{Introducción}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org02fe20f}]{Respuesta natural y forzada}
\begin{itemize}
\item El método de resolución analiza el circuito en dos etapas:
\begin{itemize}
...
\label{sec:orgea524b5}
\begin{frame}[label={sec:org59d19ec}]{Circuito básico}
\begin{center}
...
\begin{frame}[label={sec:orgf4e9649}]{Respuesta natural (t > 0)}
\begin{center}
...
\begin{frame}[label={sec:org6193939}]{Solución}
...
\begin{frame}[label={sec:org3d01b4a}]{Parámetros}
\begin{columns}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orge5571bf}]{Posibles soluciones}
\[
...
\begin{frame}[label={sec:orgd5a2be9}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:orgfde5cdf}]{Circuito Sobreamortiguado (\(\alpha > \omega_0\))}
\[
...
\begin{frame}[label={sec:org836a6d3}]{Amortiguamiento Crítico (\(\alpha = \omega_0\))}
\[
...
\begin{frame}[label={sec:org63f248b}]{Circuito Subamortiguado (\(\alpha < \omega\))}
\[
...
\begin{frame}[label={sec:org40812ef}]{Valores Importantes}
\begin{center}
...
\begin{frame}[label={sec:orgbc4f511}]{Valores Importantes}
\begin{center}
\includegraphics[height=0.6\textheight]{figs/RespuestaEscalon_SegundoOrden.png}
\end{center}
...
\begin{frame}[label={sec:org9e1705c}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:org92c715a}]{Derivadas en \(t = 0^+\)}
\begin{center}
...
\begin{frame}[label={sec:org127f199}]{Respuesta Completa}
...
\begin{frame}[label={sec:org42f5e2a}]{Ejemplo de Respuesta Completa}
...
\label{sec:org4fbe66b}
\begin{frame}[label={sec:org643c9fb}]{Circuito básico}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/transitorio_circuitoRLC_paralelo.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgbba09f6}]{Respuesta natural (t > 0)}
\begin{center}
...
\begin{frame}[label={sec:orgfa97980}]{Solución}
\begin{block}{Ecuación diferencial}
\[
  \diff[2]{u}{t} + \frac{G}{C} \diff{u}{t} + \frac{1}{LC} u = 0
\]
...
\begin{frame}[label={sec:org52a63a3}]{Parámetros}
\begin{block}{Ecuación característica}
\begin{columns}
\begin{column}{.5\columnwidth}
\begin{align*}
...
\begin{frame}[label={sec:orgb58e959}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:orgb86648c}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:org5e41d5c}]{Condiciones Iniciales}
\begin{block}{Dos constantes a determinar}
Son necesarias dos tipos de condiciones iniciales:
...
\begin{frame}[label={sec:org1707934}]{Derivadas en \(t = 0^+\): ejemplo RLC paralelo}
\begin{center}
...
\begin{frame}[label={sec:org34c1107}]{Respuesta Completa}
Las condiciones iniciales deben evaluarse teniendo en cuenta la respuesta forzada (si existe).
\begin{align*}
...
\begin{frame}[label={sec:org5bdbfc2}]{Ejemplo de Respuesta Completa}
