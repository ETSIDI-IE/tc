...
\label{sec:org402850a}

\subsection{¿Qué es el régimen transitorio?}
\label{sec:org1fbc475}
\begin{frame}[label={sec:org3f82329}]{Permanente y Transitorio}
...
\begin{frame}[label={sec:org626d36a}]{Acumulación de Energía}
...
\label{sec:org55c9c97}

\begin{frame}[label={sec:org4ac02d0}]{Ecuaciones integro-diferenciales}
...
\begin{frame}[label={sec:orga2c6cfd}]{Respuesta completa de una red lineal}
...
\begin{frame}[label={sec:org9a011aa}]{Condiciones iniciales}
\begin{itemize}
...
\begin{frame}[label={sec:org8932ace}]{Resistencia}
\[
u(t) = R i(t)
\]
...
\begin{frame}[label={sec:orge46fcdc}]{Inductancia}
\[
u(t) = L \diff{i_L(t)}{t}
\leftrightarrow
...
\begin{frame}[label={sec:org1c49f8c}]{Capacidad}
\[
i(t) = C \diff{u_C(t)}{t}
\leftrightarrow
...
\]
\end{frame}
\begin{frame}[label={sec:org87d2dc7}]{Ejemplo}
...
\label{sec:org124885a}
\begin{frame}[label={sec:org0348730}]{Definición}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org3812f27}]{Respuesta natural y forzada}
\begin{itemize}
...
\label{sec:org612a4e0}

\begin{frame}[label={sec:org29b8cea}]{Circuito básico}
\begin{itemize}
...
\begin{frame}[label={sec:org02bb16d}]{Respuesta natural}
\begin{center}
...
\begin{frame}[label={sec:org9a1f1b0}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:org16abe58}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:orge9c49ed}]{Constante de tiempo}
\[
...
\begin{frame}[label={sec:orgc20570e}]{Balance Energético}
...
\begin{frame}[label={sec:org0b32c10}]{Respuesta forzada}
...
\begin{frame}[label={sec:org739c5e9}]{Respuesta forzada}
\begin{center}
...
\begin{frame}[label={sec:org526aecf}]{Condiciones iniciales}
...
\begin{frame}[label={sec:org438b358}]{Respuesta completa (ejemplo)}
\begin{align*}
  i(t) &= i_n(t) + i_\infty(t)\\
  i_n(t) &= A e^{st}\\
  i_\infty(t) &= U_0/R\\
...
\begin{frame}[label={sec:org81929c3}]{Respuesta completa}
\[
  \boxed{i(t) = \frac{U_0}{R}(1 - e^{-\frac{t}{\tau}})  }
\]
...
\begin{frame}[label={sec:org30fc641}]{Expresión general de la respuesta completa}
\[
...
\begin{frame}[label={sec:orgf490182}]{Equivalente de Thévenin}
\begin{center}
\includegraphics[height=0.85\textheight]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}
\subsection{Circuito RC paralelo}
\label{sec:org91a041b}

\begin{frame}[label={sec:org2589ac3}]{Circuito básico}
\begin{itemize}
...
\begin{frame}[label={sec:org82ccee6}]{Respuesta natural}
\begin{center}
...
\begin{frame}[label={sec:org50d452f}]{Constante de tiempo}
\begin{itemize}
...
\begin{frame}[label={sec:orgc97ae88}]{Balance Energético}
...
\begin{frame}[label={sec:orge02577f}]{Expresión general de la respuesta completa}
\[
...
\begin{frame}[label={sec:orgc9ceca5}]{Ejemplo con respuesta forzada}
\begin{center}
...
\begin{frame}[label={sec:org2bc7b8b}]{Equivalente de Norton}
\begin{center}
\includegraphics[height=0.85\textheight]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}
...
\label{sec:org90870ab}

\begin{frame}[label={sec:org6fc3643}]{Equivalente de Thévenin/Norton}
\begin{center}
\includegraphics[height=0.85\textheight]{figs/Thevenin_PrimerOrden.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org8250f2b}]{Procedimiento General}
\begin{itemize}
...
\label{sec:org4ce1eb7}
\begin{frame}[label={sec:orgbd94830}]{Introducción}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org6198f95}]{Respuesta natural y forzada}
\begin{itemize}
\item El método de resolución analiza el circuito en dos etapas:
\begin{itemize}
...
\label{sec:org183e66b}
\begin{frame}[label={sec:org32d9942}]{Circuito básico}
\begin{center}
...
\begin{frame}[label={sec:orgd665805}]{Respuesta natural (t > 0)}
\begin{center}
...
\begin{frame}[label={sec:orgd96cb12}]{Solución}
...
\begin{frame}[label={sec:org502761e}]{Parámetros}
\begin{columns}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd447e19}]{Posibles soluciones}
\[
...
\begin{frame}[label={sec:orgd49caad}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:orgd4157da}]{Circuito Sobreamortiguado (\(\alpha > \omega_0\))}
\[
...
\begin{frame}[label={sec:orgcccb2c6}]{Amortiguamiento Crítico (\(\alpha = \omega_0\))}
\[
...
\begin{frame}[label={sec:orga176bd5}]{Circuito Subamortiguado (\(\alpha < \omega\))}
\[
...
\begin{frame}[label={sec:orgba87098}]{Valores Importantes}
\begin{center}
...
\begin{frame}[label={sec:org2a17506}]{Valores Importantes}
\begin{center}
\includegraphics[height=0.6\textheight]{figs/RespuestaEscalon_SegundoOrden.png}
\end{center}
...
\begin{frame}[label={sec:org23429b2}]{Condiciones Iniciales}
...
\begin{frame}[label={sec:org44e111e}]{Derivadas en \(t = 0^+\)}
\begin{center}
...
\begin{frame}[label={sec:org9965315}]{Respuesta Completa}
...
\begin{frame}[label={sec:orgef07aa3}]{Ejemplo de Respuesta Completa}
...
\label{sec:org3fd0bdd}
\begin{frame}[label={sec:org7ebda5d}]{Circuito básico}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/transitorio_circuitoRLC_paralelo.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgaf3d2eb}]{Respuesta natural (t > 0)}
\begin{center}
...
\begin{frame}[label={sec:orgf7d9d86}]{Solución}
\begin{block}{Ecuación diferencial}
\[
  \diff[2]{u}{t} + \frac{G}{C} \diff{u}{t} + \frac{1}{LC} u = 0
\]
...
\begin{frame}[label={sec:org08b6322}]{Parámetros}
\begin{block}{Ecuación característica}
\begin{columns}
\begin{column}{.5\columnwidth}
\begin{align*}
...
\begin{frame}[label={sec:orgde36c87}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:orga92fd98}]{Tipos de Respuesta}
\begin{itemize}
...
\begin{frame}[label={sec:org6abbea9}]{Condiciones Iniciales}
\begin{block}{Dos constantes a determinar}
Son necesarias dos tipos de condiciones iniciales:
...
\begin{frame}[label={sec:orgf7763a0}]{Derivadas en \(t = 0^+\): ejemplo RLC paralelo}
\begin{center}
...
\begin{frame}[label={sec:orga1c4c0c}]{Respuesta Completa}
Las condiciones iniciales deben evaluarse teniendo en cuenta la respuesta forzada (si existe).
\begin{align*}
...
\begin{frame}[label={sec:orgb67af46}]{Ejemplo de Respuesta Completa}
